...
async function initGames() {
  ...
  // Функция для получения случайной позиции, далекой от других спрайтов
  function getRandomPositionFarFrom(
    ...
    // Если не удалось найти позицию, возвращаем центр экрана
    return { x: 390, y: 290 }
  }
  ...
  app.ticker.add(() => {
    ...
    if (Math.hypot(player.x - enemy.x, player.y - enemy.y) < 30) {
      alert('Игра окончена! Вы коснулись врага!')
      app.ticker.stop()
      restartGame()
      return
    }
